...
class Player:
    ...
    #BEGIN VARIABLE DEFINITIONS
            if packet_values[0] == "KEYVALUE":
                print packet_values[1], packet_values[2]
            ...
            if packet_values[0] == "NEWHAND":
                ...
                print "Time banks: ", float(timeBank)
...
#REQUESTKEYVALUES
                ...
                print 'bytes left = ', packet_values[1]
                our = playersDict[our_name] #Opponent instance
                opp1 = playersDict[opp1_name]
                opp2 = playersDict[opp2_name]

                reply("PUT", our_name, s, our.getWMSD())
                reply("PUT", opp1_name, s, opp1.getWMSD())
                reply("PUT", opp2_name, s, opp2.getWMSD()) 
                
                s.send("FINISH\n")
        # Clean up the socket.
        input_socket.close()

def reply(action, amount, socket, value=None):
    ...
    if action == 'FOLD' or action == 'CHECK': 
        ...
        print 'put', amount, value
        socket.send(action + ':' + str(amount) + ':' + str(value))
...
def betLogic(input_socket,board,equities,pot_size,our_seat,bot1,bot2 = None):
    ...
    if our_equity > 0.6 and our_m >= 4 and our_m <= 7:
        ...
        if our_equity < 0.46:
            ...
            if nits_raising and useStats:
                ...
                if bluffRand < 0.2 and their_bet < maxBetFold:
                    action = "BET" 
                    #max bluffing bet
        ...
        #use ev
            ...
            if our_equity >= 0.6 and not no_reraise:
                ...
                if watch_out and not call:
                    ...
                    bluffRand = random.random()
                    if bluffRand < 0.3 and their_bet <= (maxBetFold - 3*bb):
                        action = "BET" 
                        #max bluffing bet
                        maxBet = min(our_stack,max(bluffMin,maxBetFold))
    ...
    #watch out for tight players (sharks), cashcows we can exploit by making them fold
        ...
        bluffRand = random.random()
        if round_start and bluffRand <= 0.5 and not watch_out and our_equity<0.4 and not blood_in_the_water and milk_the_cow:
        ...
        elif bluffRand <= 0.5 and not watch_out and our_equity < 0.4 and not cow_raised and not blood_in_the_water and milk_the_cow:
    ...
    if action == "CALL":
        ...
        if "BET" in avail_actions:
            ...
            if (final_max - final_min)<= 1.0 :
               ...
               bet = random.randint(final_min, int((final_min + final_max)/2.0))
